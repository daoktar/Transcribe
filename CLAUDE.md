...
- **`core.py`** — Transcription engine. `transcribe_video()` loads a whisper.cpp model via pywhispercpp (auto-detects Metal on Apple Silicon), runs transcription with real-time progress via `new_segment_callback`, and returns a dict with full text, timestamped segments, and detected language. `save_txt()` handle output serialization. Note: pywhispercpp segment timestamps (`t0`/`t1`) are in centiseconds — divided by 100 to get seconds.
